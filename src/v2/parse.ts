...
export namespace TParsedType {
  ...
  export const get = <T>(x: T): TParsedType => {
    ...
    switch (typeof x) {
      ...
      case 'object':
        if (utils.isArray(x)) return TParsedType.Array
        if (utils.isAsync(x)) return TParsedType.Promise
        if (utils.isBuffer(x)) return TParsedType.Buffer
        if (utils.isDate(x)) return TParsedType.Date
        if (utils.isMap(x)) return TParsedType.Map
        if (utils.isNull(x)) return TParsedType.Null
        if (utils.isRegExp(x)) return TParsedType.RegExp
        if (utils.isSet(x)) return TParsedType.Set
  ...
  export const Literal = (x: utils.Primitive): TParsedType => {
    if (utils.isNull(x)) {
...
export const FAIL = <I>(error: TError<I>): FailedParseResult<I> => ({
  ok: false,
  error,
})
...
export const SyncParseContext = {
  of: <T extends AnyTType>(schema: T, data: unknown, options: ParseOptions | undefined): ParseContextOf<T> =>
    ParseContext.of(schema, data, {
      ...getGlobal().getOptions(),
      ...schema.options(),
      ...options,
      async: false,
    }),
