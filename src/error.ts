...
export type ToChecks<T extends TIssue> = ReadonlyArray<
  LooseStripKey<T['payload'], 'received'> & {
...
export type IssueBase<
  K extends IssueKind,
  P extends
    | objectUtils.OmitIndexSignature<objectUtils.AnyRecord>
    | undefined = undefined
> = SimplifyDeep<
  {
    readonly kind: K
...
export type InvalidLiteralIssue = IssueBase<
  EIssueKind['InvalidLiteral'],
  {
    readonly expected: {
      ...
      readonly formatted: stringUtils.Literalized
    }
    readonly received: {
      readonly value: Primitive
      readonly formatted: stringUtils.Literalized
  ...
  EIssueKind['InvalidEnumValue'],
  {
    readonly expected: {
      ...
      readonly formatted: ReadonlyArray<
        stringUtils.Literalized<string> | stringUtils.Literalized<number>
      >
    }
    readonly received: {
      readonly value: string | number
      readonly formatted:
        | stringUtils.Literalized<string>
        | stringUtils.Literalized<number>
    }
  }
>

export type InvalidThisTypeIssue = IssueBase<
  EIssueKind['InvalidThisType'],
  { readonly issues: readonly TIssue[] }
>
export type InvalidArgumentsIssue = IssueBase<
  EIssueKind['InvalidArguments'],
  { readonly issues: readonly TIssue[] }
>
export type InvalidReturnTypeIssue = IssueBase<
  EIssueKind['InvalidReturnType'],
  { readonly issues: readonly TIssue[] }
>

export type InvalidInstanceIssue = IssueBase<
  EIssueKind['InvalidInstance'],
  { readonly expected: string }
>

export type MissingKeysIssue = IssueBase<
  EIssueKind['MissingKeys'],
  { readonly keys: readonly PropertyKey[] }
>
export type UnrecognizedKeysIssue = IssueBase<
  EIssueKind['UnrecognizedKeys'],
  { readonly keys: readonly PropertyKey[] }
>

export type InvalidUnionIssue = IssueBase<
  EIssueKind['InvalidUnion'],
  { readonly issues: readonly TIssue[] }
>
...
export type InvalidDiscriminatorIssue = IssueBase<
  EIssueKind['InvalidDiscriminator'],
  {
    readonly expected: {
      ...
      readonly formatted: readonly stringUtils.Literalized[]
    }
    readonly received: {
      readonly value: Primitive
      readonly formatted: stringUtils.Literalized
...
export type InvalidIntersectionIssue = IssueBase<
  EIssueKind['InvalidIntersection']
>
...
export type InvalidStringIssue = IssueBase<
  ...
  | {
      ...
      readonly options: {
        readonly type: 'enforce' | 'disallow'
  ...
  | {
      ...
      readonly options: Readonly<
        Required<objectUtils.CamelCaseProperties<validator.IsEmailOptions>>
      >
    }
  | { readonly check: 'url' }
  | { readonly check: 'cuid' }
  ...
  | {
      readonly check: 'numeric'
      readonly options: { readonly noSymbols: boolean }
    }
  | {
...
export type InvalidDateIssue = IssueBase<
  ...
  | TChecks.Min<Date | 'now', Date>
  | TChecks.Max<Date | 'now', Date>
  | TChecks.Range<Date | 'now', Date>
>
...
export type InvalidTupleIssue = IssueBase<
  EIssueKind['InvalidTuple'],
  TChecks.Length
>
...
export type InvalidBufferIssue = IssueBase<
  EIssueKind['InvalidBuffer'],
  TChecks.Min | TChecks.Max | TChecks.Length
>
...
export type TErrorMapDict = {
  readonly [K in IssueKind]?: (
    issue: Extract<ErrorMapIssueInput, { kind: K }>
  ) => string
}
...
export const DEFAULT_ERROR_MAP: TErrorMapFn = (issue) => {
  switch (issue.kind) {
    ...
    case IssueKind.InvalidLiteral:
      return `Expected the literal value ${String(
        issue.payload.expected.formatted
      )}, got ${String(issue.payload.received.formatted)}`
    case IssueKind.InvalidEnumValue:
      return `Expected one of ${issue.payload.expected.formatted.join(
        ' | '
      )}, got ${issue.payload.received.formatted}`
    ...
    case IssueKind.MissingKeys:
      return `Missing key(s) in object: ${issue.payload.keys
        .map(stringUtils.literalize)
        .join(', ')}`
    case IssueKind.UnrecognizedKeys:
      return `Unrecognized key(s) found in object: ${issue.payload.keys
        .map(stringUtils.literalize)
        .join(', ')}`
    ...
    case IssueKind.InvalidString:
      if (issue.payload.check === 'min')
        return `String must contain ${
          issue.payload.expected.inclusive ? 'at least' : 'over'
        } ${issue.payload.expected.value} character(s)`
      if (issue.payload.check === 'max')
        return `String must contain ${
          issue.payload.expected.inclusive ? 'at most' : 'under'
        } ${issue.payload.expected.value} character(s)`
      if (issue.payload.check === 'length')
        return `String must contain exactly ${issue.payload.expected} character(s)`
      if (issue.payload.check === 'pattern')
        return `String must ${
          issue.payload.options.type === 'enforce' ? 'match' : 'not match'
        } the pattern: ${issue.payload.options.name}`
      if (issue.payload.check === 'alphanum')
        return 'String must contain only alphanumeric characters'
      if (issue.payload.check === 'email')
      ...
      if (issue.payload.check === 'iso_date')
        return 'String must be a valid ISO date'
      if (issue.payload.check === 'iso_duration')
        return 'String must be a valid ISO duration'
      if (issue.payload.check === 'base64')
        return 'String must be a valid base64 string'
      if (issue.payload.check === 'numeric') return 'String must be numeric'
      if (issue.payload.check === 'starts_with')
        return `String must start with "${issue.payload.prefix}"`
      if (issue.payload.check === 'ends_with')
        return `String must end with "${issue.payload.suffix}"`
      if (issue.payload.check === 'contains')
        return `String must contain the substring "${issue.payload.substring}"`
      return TError.assertNever(issue.payload)
    case IssueKind.InvalidNumber:
      if (issue.payload.check === 'min')
        return `Number must be ${
          issue.payload.expected.inclusive
      ...
      if (issue.payload.check === 'port')
        return 'Value must be a valid port number'
      if (issue.payload.check === 'multiple')
        return `Value must be a multiple of ${issue.payload.expected}`
      ...
      if (issue.payload.check === 'precision')
        return `Value must contain ${
          issue.payload.expected.inclusive
            ? 'less than or equal to'
            : 'less than'
        } ${issue.payload.expected.value} decimal places, got ${
          issue.payload.received
        }`
      return TError.assertNever(issue.payload)
    case IssueKind.InvalidBigInt:
      if (issue.payload.check === 'min')
        return `BigInt must be ${
          issue.payload.expected.inclusive
            ? 'greater than or equal to'
            : 'greater than'
        } ${issue.payload.expected.value}n`
      if (issue.payload.check === 'max')
        return `BigInt must be ${
          issue.payload.expected.inclusive
            ? 'less than or equal to'
      ...
      if (issue.payload.check === 'multiple')
        return `Number must be a multiple of ${issue.payload.expected}`
      return TError.assertNever(issue.payload)
    case IssueKind.InvalidDate:
      if (issue.payload.check === 'min')
        return `Date must be ${
          ...
          issue.payload.expected.value === 'now'
            ? 'the current time'
            : issue.payload.expected.value.toISOString()
        }`
      if (issue.payload.check === 'max')
        return `Date must be ${
          issue.payload.expected.inclusive
    ...
    case IssueKind.InvalidArray:
      if (issue.payload.check === 'min')
        return `Array must contain ${
          issue.payload.expected.inclusive ? 'at least' : 'over'
        } ${issue.payload.expected.value} item(s)`
      if (issue.payload.check === 'max')
        return `Array must contain ${
          issue.payload.expected.inclusive ? 'at most' : 'under'
        } ${issue.payload.expected.value} item(s)`
    ...
    case IssueKind.InvalidBuffer:
      if (issue.payload.check === 'min')
        return `Buffer must contain ${
          issue.payload.expected.inclusive ? 'at least' : 'over'
        } ${issue.payload.expected.value} byte(s)`
      if (issue.payload.check === 'max')
        return `Buffer must contain ${
          issue.payload.expected.inclusive ? 'at most' : 'under'
        } ${issue.payload.expected.value} byte(s)`
      if (issue.payload.check === 'length')
        return `Buffer must contain exactly ${issue.payload.expected} byte(s)`
      return TError.assertNever(issue.payload)
...
export const resolveErrorMaps = (
  maps: ReadonlyArray<TErrorMap | undefined>
): TErrorMapFn => {
  return (issue) => {
    const msgs = [...maps]
      .reverse()
      .map((map) =>
        (typeof map === 'function'
          ? map
          : (issue: ErrorMapIssueInput): string | undefined =>
              (map?.[issue.kind] as TErrorMapFn | undefined)?.(issue))(issue)
      )
...
export class TError<
  I,
  T extends AnyTType<unknown, I> = AnyTType<unknown, I>
> extends Error {
  ...
  static fromContext<T extends AnyTType>(
